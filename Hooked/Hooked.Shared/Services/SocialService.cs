...
namespace Hooked.Shared.Services
{
    public sealed class SocialService : ISocialService
    {
        ...
        public async Task<SocialCatchDetailDto?> GetCatchDetailAsync(Guid catchId, Guid viewerUserId, CancellationToken cancellationToken = default)
        {
            if (catchId == Guid.Empty)
            {
                return null;
            ...
            return new SocialCatchDetailDto(
                catchRecord.Id,
                catchRecord.UserId,
